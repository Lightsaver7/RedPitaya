...
class DACCb : public ConfigCallback {
    ...
    void configError(ConfigStreamClient*, [[maybe_unused]] std::string host, [[maybe_unused]] int error) override {
        if (m_terminate)
            *m_terminate = true;
    }

    void dacServerStoppedMemError(ConfigStreamClient*, [[maybe_unused]] std::string host) override {
        if (m_terminate)
            *m_terminate = true;
    }
    void dacServerStoppedMemModify(ConfigStreamClient*, [[maybe_unused]] std::string host) override {
        if (m_terminate)
            *m_terminate = true;
    }
    void dacServerStoppedConfigError(ConfigStreamClient*, [[maybe_unused]] std::string host) override {
        if (m_terminate)
            *m_terminate = true;
    }
    void configErrorFileMissed(ConfigStreamClient*, [[maybe_unused]] std::string host) override {
        if (m_terminate)
            *m_terminate = true;
    }
...
auto DACStreamClient::Impl::runClient(DACStreamClient* client, std::string host, uint32_t size, dac_channels_t activeChannels) -> void {
    ...
    switch (m_dataMode) {
        ...
        case MEM_STREAM_MODE: {
            m_dac_manger = CDACStreamingManager::Create(activeChannels,
    ...
    m_dac_manger->getBufferManager()->generateBuffersEmptyDAC(activeChannels, memoryManager->getRegions(uio_lib::MM_DAC), DataLib::sizeHeader());
    ...
    m_dac_asionet->sendNotify.connect([&](std::error_code err, size_t) {
