...
auto connectConfigServer(std::shared_ptr<ClientNetConfigManager> cl, ClientOpt::Options& option) -> bool {
    ...
    cl->connectToServers(option.hosts, NET_CONFIG_PORT);
...
auto getConfigItem(std::shared_ptr<ClientNetConfigManager> cl, ClientOpt::Options& option) -> bool {
    std::atomic<int> get_counter;
    cl->getNewSettingsItemNofiy.connect([&](std::string host) {
        ...
        if (option.verbous)
            aprintf(stdout, "%s Get settings from: %s\n", getTS(": ").c_str(), host.c_str());
        CStreamSettings* s = cl->getLocalSettingsOfHost(host);
        auto str = s->getValue(option.config_item_name);

        if (option.verbous) {
            aprintf(stdout, "\n========================================================\n");
            aprintf(stdout, "Host:\t\t\t%s\n", host.c_str());
            aprintf(stdout, "%s=%s", option.config_item_name.c_str(), str.c_str());
            aprintf(stdout, "\n========================================================\n");
        } else {
            aprintf(stdout, "%s\n", str.c_str());
        }

        if (option.config_item_write) {
            createDirTreeConfig("configs");
            std::string file = "configs/config_" + host + ".json";
            if (option.verbous)
                aprintf(stdout, "%s Save configuration from %s to %s\n", getTS(": ").c_str(), host.c_str(), file.c_str());
    ...
    for (auto& host : connected_hosts) {
        if (option.verbous)
            aprintf(stdout, "%s Send configuration request: %s\n", getTS(": ").c_str(), host.c_str());
        if (!cl->requestConfigVariable(host, option.config_item_name)) {
            get_counter--;
        }
    }
    while (get_counter > 0) {
...
auto setConfigItem(std::shared_ptr<ClientNetConfigManager> cl, ClientOpt::Options& option) -> bool {
    std::atomic<int> set_counter;
    cl->successSendConfigNofiy.connect([&](std::string host) {
        const std::lock_guard<std::mutex> lock(g_mutex);
        if (option.verbous)
    ...
    for (auto& host : connected_hosts) {
        if (option.verbous)
            aprintf(stdout, "%s Send configuration to: %s\n", getTS(": ").c_str(), host.c_str());
        if (!cl->sendConfigVariable(host, option.config_item_name, option.config_item_value)) {
            set_counter--;
        }
    }
    while (set_counter > 0) {
...
auto startConfig(std::shared_ptr<ClientNetConfigManager> cl, ClientOpt::Options& option) -> bool {
    ...
    if (option.config_item_name != "" && option.config_item_value == "") {
        return getConfigItem(cl, option);
    }

    if (option.config_item_name != "" && option.config_item_value != "") {
        return setConfigItem(cl, option);
    }
