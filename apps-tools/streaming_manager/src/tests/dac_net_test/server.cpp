...
int main(int argc, char* argv[]) {
    ...
    server->addHandler(CDACAsioNetController::Events::CONNECTED, [](std::string host) { std::cout << "CLIENT CONNECTED " << host << "\n"; });
    server->addHandler(CDACAsioNetController::Events::DISCONNECTED, [](std::string host) { std::cout << "CLIENT DISCONNECTED " << host << "\n"; });
