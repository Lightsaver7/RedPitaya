...
int rpApp_BaDataAnalysisFFT(const rp_ba_buffer_t& buffer, uint32_t size,
                            float _freq,  // 2*pi*f
                            int decimation, float* gain, float* phase_out, float input_threshold) {
    ...
    if (size > 0) {
        ...
        for (size_t i = 1; i < size; i++) {
            if (u1_max < buffer.ch1[i])
                u1_max = buffer.ch1[i];
            if (u2_max < buffer.ch2[i])
                u2_max = buffer.ch2[i];
            if (u1_min > buffer.ch1[i])
                u1_min = buffer.ch1[i];
            if (u2_min > buffer.ch2[i])
                u2_min = buffer.ch2[i];
        }
        if ((u1_max - u1_min) < input_threshold)
            ret_value = RP_EIPV;
        if ((u2_max - u2_min) < input_threshold)
            ret_value = RP_EIPV;
    }
...
int rpApp_BaDataAnalysis(const rp_ba_buffer_t& buffer, uint32_t size, float samplesPerSecond, float _freq, float samples_period, float* gain, float* phase_out,
                         float input_threshold) {
    int ret_value = RP_OK;
    ...
    *gain = sig2_rms / sig1_rms;
    ...
    if ((max_ch1 - min_ch1) < input_threshold)
        ret_value = RP_EIPV;
    if ((max_ch2 - min_ch2) < input_threshold)
        ret_value = RP_EIPV;
...
int rpApp_BaGetAmplPhase(rp_ba_logic_t mode, float _amplitude_in, float _dc_bias, int _periods_number, rp_ba_buffer_t& _buffer, float* _amplitude,
                         float* _phase, float _freq, float _probe, float _input_threshold) {
    float gain = 0;
    float phase_out = 0;
    int ret = 0;
    int acq_size;
    ...
    if (mode == RP_BA_LOGIC_TRAP) {
        float phase[2];
        ret = analysisTrap(_buffer.ch1, _buffer.ch2, _freq, decimation, adc_rate, _input_threshold, &phase[0], &phase[1], &gain, &phase_out);
    }

    if (mode == RP_BA_LOGIC_FFT) {
        ret = analysisFFT(_buffer.ch1, _buffer.ch2, _freq, decimation, &gain, &phase_out, _input_threshold);
    }
    ...
    *_phase = phase_out;
    ...
    if (std::isnan(*_amplitude) || std::isinf(*_amplitude))
        ret = RP_EOOR;
    return ret;
}
