...
auto CWaveWriter::Impl::write(SBufferPack* _pack, std::iostream* _memory) -> bool {
    ...
    m_numChannels = 0;
    size_t maxSamples = 0;
    auto maxBitBySample = SBufferPack::getBitsCount(RP_F_ui8_Bit);
    ...
    auto get16Bit = [&](uint8_t ch, size_t pos) -> uint16_t {
        if (channelsBits[ch] == RP_F_ui8_Bit) {
        ...
        if (channelsBits[ch] == RP_F_ui16_Bit) {
            if (channelsSamples[ch] > pos) {
    ...
    auto get32Bit = [&](uint8_t ch, size_t pos) -> float {
        ...
        if (channelsBits[ch] == RP_F_f32_Bit) {
            if (channelsSamples[ch] > pos) {
        ...
        if (channelsBits[ch] == RP_F_d64_Bit) {
            if (channelsSamples[ch] > pos) {
