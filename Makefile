...
BUILD_UNIT_TESTS ?= ON
VERBOSE = OFF
CMAKEVAR=-DINSTALL_DIR=$(abspath $(INSTALL_DIR)) -DCMAKE_BUILD_TYPE=$(BUILD_MODE)  -DVERSION=$(VERSION) -DLINUX_VER=$(LINUX_VER) -DBUILD_NUMBER=$(BUILD_NUMBER) -DREVISION=$(REVISION) -DCMAKE_VERBOSE_MAKEFILE:BOOL=$(VERBOSE) -DBUILD_UNIT_TESTS:BOOL=$(BUILD_UNIT_TESTS)
################################################################################
#
################################################################################
